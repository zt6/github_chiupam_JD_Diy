...
async def myuntempblockcookie(event):
    ...
    for config in configs:
        if config.find('TempBlockCookie') != -1 and configs[configs.index(config) + 1].find(';;\n') == -1 and config.find('举例') == -1:
            m, temp = re.findall(r'\d', config), config
            break
        elif config.find('AutoDelCron') != -1:
            break
    if m == []:
        ...
        for n in m:
            expired_cookie = checkCookie2(cookies[int(n) - 1])
            if not expired_cookie:
                del(m[m.index(n)])
        if m != []:
            x = ' '.join(m)
            with open(path, 'w', encoding='utf-8') as f2:
                del(configs[-1])
                print(''.join(configs), file=f2)
            configs[configs.index(temp)] = f'TempBlockCookie="{x}"\n'
        else:
            configs[configs.index(temp)] = f'TempBlockCookie=""\n'
            with open(path, 'w', encoding='utf-8') as f2:
                del(configs[-1])
                print(''.join(configs), file=f2)
